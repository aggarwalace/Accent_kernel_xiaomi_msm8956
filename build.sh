...
TOOLCHAIN=/home/ace/octos/prebuilts/gcc/linux-x86/aarch64
...
BASE="Accent-kernel-"
CUR_VER="V1"
ACCENT_VER="$BASE$CUR_VER"
 
# Variables

DEFCONFIG="accent_hydrogen_defconfig"
export LOCALVERSION=~`echo $ACCENT_VER`
export CROSS_COMPILE=$TOOLCHAIN/aarch64-linux-android-*
export ARCH=arm64
export KBUILD_BUILD_USER="aggarwalace"
export KBUILD_BUILD_HOST="SheershBansal"

function make_Accent {
		echo -e "$green*******************************************************"
		echo "                  Compiling $ACCENT_VER	              "
		echo -e "*****************************************************"
		echo
		make $DEFCONFIG
		make -j4
...
function make_zip {
		...
		cd $OUT_DIR
		rm -f '*.zip'
		zip -yr Accent_hydrogen_helium_`echo $CUR_VER`.zip *
		mv Accent_hydrogen_helium`echo $CUR_VER`.zip $OUT_ZIP
